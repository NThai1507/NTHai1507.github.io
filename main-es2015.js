...
let WeatherComponent = class WeatherComponent {
    ...
    getWeather() {
        this.isloading = true;
        this.getMessage();
        this.weatherService.getTemp(this.location).subscribe(data => {
            this.data = data['main'].temp_max;
            this.isloading = false;
            this.getMessage();
        }, _error => {
            this.message = 'your location is invalid!';
        });
    }
    getBooks() {
        this.weatherService.getBooks().subscribe(data => {
            this.books = data;
...
let WeatherService = class WeatherService {
    ...
    getTemp(name) {
        const uri = "https://api.openweathermap.org/data/2.5/weather?APPID=008bd33563e0e2a0b10907eacd7099e6&q=" + name;
        var ress = this.http.get(uri);
        console.log(ress);
        return ress;
    }
    getBooks() {
        const uri = "http://192.168.225.163:8000/books";
...
let YComponent = class YComponent {
    ...
    onSubmit(form) {
        this.signInService.sendPost(form.value).subscribe(data => {
            console.log(data);
        }, _error => {
            console.log("Error");
        });
